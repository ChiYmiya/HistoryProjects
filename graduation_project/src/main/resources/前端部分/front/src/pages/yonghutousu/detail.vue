  ...
  <div>
    ...
    >
        ...
        >
          ...
          >
            ...
            <el-button
              ...
              v-if="btnAuth('yonghutousu', '私聊') && detail.id != mid"
              @click="chatClick"
              >联系TA</el-button
            >
            <el-button
      ...
      <el-tabs
        ...
        type="border-card"
      >
        <el-tab-pane label="投诉原因" name="first">
          <div v-html="detail.tousuyuanyin"></div>
        </el-tab-pane>
      </el-tabs>
    </div>
    <div
...
export default {
  ...
  methods: {
    ...
    async onAcross(
      ...
      localStorage.setItem("crossTable", `yonghutousu`);
    ...
    // 修改
    editClick() {
      this.$router.push(`/index/yonghutousuAdd?type=edit&&id=${this.detail.id}`);
    },
    // 删除
    async delClick() {
      await this.$confirm("是否删除此用户投诉？")
        .then((_) => {
          this.$http.post("yonghutousu/delete", [this.detail.id]).then(async (res) => {
            if (res.data.code == 0) {
              this.$message({
                type: "success",
                message: "删除成功!",
