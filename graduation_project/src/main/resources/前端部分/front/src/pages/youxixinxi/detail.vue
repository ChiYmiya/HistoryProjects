  ...
  <div>
    ...
    >
        ...
        >
          ...
          >
            ...
            >
              ...
              ></i
              ><span
                :style="{ color: '#999', fontSize: '14px', textDecoration: 'underline' }"
                >点我收藏({{ detail.storeupnum }})</span
              >
            </div>
            <div
              @click="storeup(-1)"
          ...
          >
            ...
            >
              {{
                detail.sfsh == "是" ? "通过" : detail.sfsh == "否" ? "不通过" : "待审核"
              }}
            </div>
          </div>
          <div
          ...
          >
            ...
            <el-button
              ...
              v-if="btnAuth('youxixinxi', '私聊') && detail.id != mid"
              @click="chatClick"
              >联系TA</el-button
            >
            <el-button
              ...
              v-if="
                btnAuth('youxixinxi', '审核') && (detail.sfsh == '待审核' || !detail.sfsh)
              "
              @click="onSH"
              type="warning"
              >审核</el-button
      ...
      >
        ...
        >
          <i
            :style="{ color: '#fff', margin: '0 3px', fontSize: '16px' }"
            class="iconfont icon-dianzan"
          ></i>
          <span :style="{ color: '#fff', margin: '0 3px', fontSize: '14px' }"
            >赞一下({{ detail.thumbsupnum }})</span
          >
        </div>
        <div
          :style="{
        ...
        >
          <i
            :style="{ color: '#fff', margin: '0 3px', fontSize: '16px' }"
            class="iconfont icon-iconfontzan"
          ></i>
          <span :style="{ color: '#fff', margin: '0 3px', fontSize: '14px' }"
            >踩一下({{ detail.crazilynum }})</span
          >
        </div>
        <div
          :style="{
      ...
      >
        <el-tab-pane label="游戏介绍" name="first">
          <div v-html="detail.youxijieshao"></div>
        </el-tab-pane>
        <el-tab-pane label="评论" name="second">
          <el-form
            class="add comment"
            :style="{ padding: '15px', margin: '0 0 20px' }"
            :model="form"
            :rules="rules"
            ref="form"
          >
  ...
  methods: {
    ...
    storeup(type) {
      if (type == 1 && !this.isStoreup) {
        ...
        this.storeupParams.inteltype = this.detail.youxifenlei;
      ...
      axios
        ...
        .then(
          ...
          (err) => {
            axios
              .get(
                (location.href.split(this.$config.name).length > 1
                  ? location.href.split(this.$config.name)[0]
                  : "") +
                  this.$config.name +
    ...
    discussDel(id) {
      this.$confirm("是否删除此评论？")
        .then((_) => {
          this.$http.post("discussyouxixinxi/delete", [id]).then((res) => {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.form.avatarurl = localStorage.getItem("frontHeadportrait")
            ...
            : "";
          this.$http.post("discussyouxixinxi/add", this.form).then((res) => {
            if (res.data.code == 0) {
              this.form.content = "";
              this.form.score = 0;
    ...
    // 删除
    async delClick() {
      await this.$confirm("是否删除此游戏信息？")
        .then((_) => {
          this.$http.post("youxixinxi/delete", [this.detail.id]).then(async (res) => {
            if (res.data.code == 0) {
              this.$http
                .get("storeup/list", {
                  params: {
...
.detail-preview .detail.el-tabs /deep/ .el-tabs__header .el-tabs__item:hover {
  ...
  background: none;
  border-color: rgb(255, 133, 161) #e9e9e9;
  border-width: 6px 1px 0 0;
  border-style: solid;
}
