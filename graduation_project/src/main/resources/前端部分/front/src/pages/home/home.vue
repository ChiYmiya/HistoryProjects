<template>
  ...
  >
    <div
      id="animate_recommendwangzhangonggao"
    ...
    >
      ...
      <div
        ...
        :style="{
          ...
          margin: '0px auto',
          textAlign: 'left',
          background: 'url() no-repeat left top / auto 42px',
          height: '120px',
        }"
      >
        <span
      ...
      <div
        v-if="wangzhangonggaoRecommend.length"
      ...
      >
        <div
          v-for="(item, index) in wangzhangonggaoRecommend"
          :key="index"
          @click="toDetail('wangzhangonggaoDetail', item)"
      ...
      <div
        ...
        :style="{
          ...
          top: '0',
          background:
            'url(http://codegen.caihongy.cn/20231010/b345185c76ac4a77bf4cd6f6171a72ed.png) no-repeat center top / 100% 100%',
          width: '240px',
          lineHeight: '73px',
          position: 'absolute',
          height: '80px',
      ...
      >
        ...
        <i
          :style="{ color: '#333', fontSize: '16px', display: 'none' }"
          class="icon iconfont icon-jiantou09"
        ></i>
      </div>
    </div>
    ...
    <!-- 游戏推荐 -->
    <div
      id="animate_recommendyouxixinxi"
      class="recommend animate__animated"
      :style="{
        margin: '40px auto 20px',
        borderColor: '#ddd',
    ...
    >
      ...
      <div
        v-if="youxixinxiRecommend.length"
        class="list list16 index-pv1"
        :style="{
          padding: '0 12%',
          margin: '20px 0 0',
      ...
      >
        <div
          v-for="(item, index) in youxixinxiRecommend"
          :key="index"
          @click="toDetail('youxixinxiDetail', item)"
          class="list-item animation-box"
        >
          <div
            :style="{
          ...
          >
            ...
            >
              <div class="title">{{ item.youximingcheng }}</div>
              <div class="title">{{ item.youxifenlei }}</div>
              <div
                :style="{
                  padding: '0 10px',
                  borderColor: 'red',
            ...
            <div class="desc" v-html="item.shiherenqun"></div>
          </div>
        </div>
      </div>

      <div
        @click="moreBtn('youxixinxi')"
        :style="{
          border: '0px solid #fff',
          cursor: 'pointer',
          padding: '0 20px 0 0',
  ...
  methods: {
    ...
    handleScroll() {
      let arr = [
        ...
        { id: "animate_recommendyouxixinxi", css: "animate__" },
        { id: "animate_recommendwangzhangonggao", css: "animate__" },
    ...
    getList() {
      ...
      if (localStorage.getItem("frontToken")) {
        autoSortUrl = "youxixinxi/autoSort2";
      }
      data = {
        page: 1,
        limit: 8,
      ...
      this.$http.get(autoSortUrl, { params: data }).then((res) => {
        if (res.data.code == 0) {
          this.youxixinxiRecommend = res.data.data.list;

          // 列表样式五
        }
      });
      autoSortUrl = "wangzhangonggao/autoSort";
      data = {
        page: 1,
        limit: 8,
      };
      this.$http.get(autoSortUrl, { params: data }).then((res) => {
        if (res.data.code == 0) {
          this.wangzhangonggaoRecommend = res.data.data.list;
...
.home-preview {
  ...
  .search .select /deep/ .el-input__inner {
    ...
    box-shadow: 0 0 0px rgba(64, 158, 255, 0.3);
    outline: none;
    color: #808080;
    background: #f5f5f5;
    width: 180px;
  ...
  .search .input /deep/ .el-input__inner {
    ...
    outline: none;
    color: #808080;
    background: url(http://codegen.caihongy.cn/20231010/e6f200cdf25b457cae56c7a886461d70.png)
        no-repeat 20px center / auto 36px,
      #f5f5f5;
    width: 380px;
    font-size: 16px;
    height: 60px;
