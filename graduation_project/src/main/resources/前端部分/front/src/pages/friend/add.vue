  ...
  >
    ...
    >
      ...
      <el-form-item
        ...
        label="图片"
        v-if="type != 'cross' || (type == 'cross' && !ro.picture)"
        prop="picture"
      >
        <file-upload
          tip="点击上传图片"
...
export default {
  data() {
    ...
    return {
      ...
      rules: {
        ...
        fid: [{ required: true, message: "好友用户ID不能为空", trigger: "blur" }],
        name: [{ required: true, message: "名称不能为空", trigger: "blur" }],
        picture: [{ required: true, message: "图片不能为空", trigger: "blur" }],
        role: [],
        tablename: [],
        alias: [],
        type: [{ validator: this.$validate.isIntNumer, trigger: "blur" }],
      },
      centerType: false,
    };
  },
  ...
  methods: {
    ...
    info() {
      this.$http
        .get(`friend/detail/${this.$route.query.id}`, { emulateJSON: true })
        .then((res) => {
          if (res.data.code == 0) {
